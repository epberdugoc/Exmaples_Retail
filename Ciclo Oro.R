...
ruta = "D:\\Copia HMEDINA\\Documents\\02. Mercadeo\\05. Victor Antequera\\Ciclos\\01. Colombia\\Oro\\2019"
setwd(ruta)
...
### ESTAS SON LAS PIEZAS QUE CORRESPONDEN A CADA PORCENTAJE DE PAGO

PIEZAS = c("MONEDA","CONTENEDOR","CERTIFICADO")

### TABLA CON LOS RESUMENES PARA CADA BD

RESUMEN = data.frame("ITEM" = "", "CANTIDAD" = 0, 
                     "OBSERVACIONES" = c(paste("BONO DE $80.000  VALIDO HASTA ",ceiling_date(Sys.Date() %m+% months(1), "months") %m-% days(1)),
                                         paste("BONO DE $100.000  VALIDO HASTA ",ceiling_date(Sys.Date() %m+% months(1), "months") %m-% days(1)),
                                         paste("BONO DE $150.000  VALIDO HASTA ",ceiling_date(Sys.Date() %m+% months(1), "months") %m-% days(1))), 
                     stringsAsFactors = F)
...
attach(BD_ORO)
for (i in 1:3) {
  DATOS_TEMP = cbind(BD_ORO[(PORC_PAGO>=LIMITES[i,1])&(PORC_PAGO<LIMITES[i,2]),1:4],
                     data.frame("PIEZA" =  PIEZAS[i]) ,
                     BD_ORO[(PORC_PAGO>=LIMITES[i,1])&(PORC_PAGO<LIMITES[i,2]),5:19])
  
  RESUMEN$ITEM[i] =  PIEZAS[i]
