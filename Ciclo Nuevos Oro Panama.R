...
ruta = "D:\\OneDrive - Muebles Jamar\\hmedina\\Documents\\02. Mercadeo\\05. Victor Antequera\\Ciclos\\02. Panama\\Nuevos Oro\\2019"
setwd(ruta)
...
system.time({
  ...
  CICLOS_ANTERIORES =  sqlQuery(DWJAMAR_Connection,"SELECT CLIENTE_CODIGO, CAMPO1 CONSECUTIVO
                                ...
                                WHERE CLIENTE_CAMPANA  = 'BD_NUEVO_ORO_PANAMA'", dec=".")
...
SQL_NORO = paste("SELECT CLIENTES.CLIENTE_CODIGO CLIENTE,
       ...
       (SELECT CARTERA.CART_TRAMOINI TRAMO
         ...
         WHERE UNIDAD_TIEMPO.UNIDAD_CODIGO = CARTERA.CART_UNIDAD
           ...
           AND CARTERA.CART_CLIENTE = CLIENTES.CLIENTE_CODIGO) TRAMO_INICIAL,
 ...
 WHERE CLIENTES.CLIENTE_ZONA = ZONA_GEOGRAFICA.ZONA_CODIGO
   AND CLIENTES.CLIENTES_SEGMENTO IN ('ORO')
...
system.time({
  DWJAMAR_Connection =  odbcConnect(servidor, uid = usuario, pwd = clave)
  BD_NORO =  sqlQuery(DWJAMAR_Connection,SQL_NORO, dec=".")
  odbcClose(DWJAMAR_Connection)
}
)
...
BD_NORO = BD_NORO[is.na(FECHA_FUE_ORO)==T,1:17]
...
SUBIR = ifelse(is.na(CLIENTE)==FALSE,paste0("insert into CLIENTES_CAMPANAS (CLIENTE_CODIGO, CLIENTE_CAMPANA, CAMPO1, CAMPO2) values ('",CLIENTE,"','BD_NUEVO_ORO_PANAMA','","','",format(Sys.Date(),"%B-%Y"),"')"))
